...
| `recompHook(target, callback)` | `AddressLike, (ctx: NativeHookContext) => any` | `boolean` |
...
- `recompHook(target, callback)` — 等价于 `hook(target, callback, Hook.RECOMP)`
- `unhook(target)`
...
// stealth=2 (recomp)
recompHook(openPtr, function(ctx) { ctx.orig() })
```
